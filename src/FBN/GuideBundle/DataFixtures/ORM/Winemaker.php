...
use FBN\GuideBundle\Entity\Winemaker as Wmkr;
...
class Winemaker extends AbstractFixture implements OrderedFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        foreach ($descriptions as $i => $description) {
            ...
            $repository->translate($vnr[$i], 'description', 'fr', $descriptionsfr[$i]);
            $vnr[$i]->setPublication(true);

            $vnr[$i]->setArticleOwner($this->getReference('user-0'));
            $vnr[$i]->setArticleAuthor($this->getReference('user-0')->getAuthorName());
